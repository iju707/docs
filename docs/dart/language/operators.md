...
이중 많은 것을 [클래스 멤버의 연산자](methods.md#operators)로 구현할 수 있습니다.
...
2. y를 인자로 x의 `==` 함수를 수행하여 결과를 반환합니다. (맞다. `==`와 같은 연산자는 첫번째 피연산자에서 실행되는 함수입니다. 자세한 정보는 [연산자](methods.md#operators)를 참고하세요)
