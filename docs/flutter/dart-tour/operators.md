...
이중 많은 것을 [클래스 멤버의 연산자](classes.md#operators)로 구현할 수 있습니다.
...
| 논리 OR | `||` |
| null일 경우 | `??` |
| 조건식 | `expr1 ? expr2 : epxr3` |
| 캐스케이드 | `..` `?..` |
| 할당 | `=` `*=` `/=` `-=` `&=` `^=` 등 |

:::warning
연산자 우선순위는 Dart 파서의 동작에 대한 근사치 입니다.
이에 대한 확실한 답변은 [Dart 언어 사양](https://dart.dev/guides/language/spec)을 참고하세요.
:::
...
[연산자 테이블](operators.md)에서 각 연산자는 아래의 행에 있는 연산자보다 우선순위를 가집니다.
...
2. y를 인자로 x의 `==` 함수를 수행하여 결과를 반환합니다. (맞다. `==`와 같은 연산자는 첫번째 피연산자에서 실행되는 함수입니다. 자세한 정보는 [연산자](classes.md#operators)를 참고하세요)
...
| 연산자 | 의미 |
| :- | :- |
| `as` | 타입변환 ([라이브러리 접두어](https://dart.dev/guides/language/language-tour#specifying-a-library-prefix) 지정으로도 사용) |
...
`.`, `?.`, `..` 연산자에 대한 자세한 정보는 [클래스](./classes.md)를 참고하세요.
